...
class HomeController
{
    ...
    {
        require_admin();
        $rows = db()->query("SELECT * FROM home_why_points ORDER BY sort_order ASC")->fetchAll();

        foreach ($rows as &$r) {
            $r["icon"] = asset_url($r["icon"]);
        }
    ...
    public static function createTestimonial(): void
    {
        ...
        if(!$name||!$message) json_response(["success"=>false,"message"=>"name & message required"],400);
